...
TEST_VAULT_N8N_ENCRYPTION_KEY = os.urandom(32).hex()
...
def test_encrypt_decrypt_success() -> None:
    ...
    encrypted_data = encrypt_data(original_data, TEST_VAULT_N8N_ENCRYPTION_KEY)
    decrypted_data = decrypt_data(encrypted_data, TEST_VAULT_N8N_ENCRYPTION_KEY)
...
def test_decrypt_with_corrupted_data() -> None:
    ...
    with pytest.raises(DecryptionError, match="Ошибка аутентификации данных"):
        decrypt_data(corrupted_encrypted_data, TEST_VAULT_N8N_ENCRYPTION_KEY)
...
def test_decrypt_with_invalid_format() -> None:
    ...
    with pytest.raises(DecryptionError, match="Некорректный формат зашифрованных данных"):
        decrypt_data(invalid_format_data, TEST_VAULT_N8N_ENCRYPTION_KEY)
    ...
    with pytest.raises(DecryptionError, match="Ошибка декодирования base64 частей зашифрованных данных"):
        decrypt_data(invalid_base64_data, TEST_VAULT_N8N_ENCRYPTION_KEY)