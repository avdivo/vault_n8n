...
1.  **Выполните команду в терминале:**
    ...
    > ✨ **Важно:** При первом запуске скрипт сгенерирует и выведет в консоль `AUTH_TOKEN` и `ENCRYPTION_KEY`. Обязательно скопируйте и сохраните их в надежном месте. Эти данные также будут сохранены в файл `.env` в текущей директории.
...
2.  **Запустите скрипт для разработки:**
    ...
    ./start-dev.sh
    ```
    > ✨ **Важно:** При первом запуске скрипт сгенерирует и выведет в консоль `AUTH_TOKEN` и `ENCRYPTION_KEY`, если они отсутствуют в файле `.env`. Обязательно скопируйте и сохраните их.
